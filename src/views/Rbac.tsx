...
function RBACTab() {
  const { t, i18n } = useTranslation();
  const isZh = i18n.language.toLowerCase().startsWith('zh');
  const { activeServer, apiJson, noLicense, featureNotEnabled, resetGate } = useApiFetch();
  ...
  const loadBase = useCallback(async () => {
    if (!activeServer || featureNotEnabled || noLicense) return;
    setError(null);

    try {
      const [orgsData, tokensData] = await Promise.all([
        apiJson('/api/v1/rbac/organizations'),
        apiJson('/api/v1/auth/tokens'),
      ]);
      setOrgs(Array.isArray(orgsData?.organizations) ? orgsData.organizations : []);
      setTokens(Array.isArray(tokensData?.tokens) ? tokensData.tokens : []);
    } catch (err: any) {
      setError(err.message || t('views.rbac.loadFailed'));
    }
  }, [activeServer, apiJson, featureNotEnabled, noLicense, t]);

  const loadTeamsForOrg = useCallback(async (orgId: Id) => {
    if (!activeServer) return;
    const data = await apiJson(`/api/v1/rbac/organizations/${orgId}/teams`);
    const rows = Array.isArray(data.teams) ? data.teams as Team[] : [];
    setTeams(prev => [...prev.filter(t => t.organization_id !== orgId), ...rows]);
  }, [activeServer, apiJson]);

  const loadRolesForTeam = useCallback(async (teamId: Id) => {
    if (!activeServer) return;
    const data = await apiJson(`/api/v1/rbac/teams/${teamId}/roles`);
  ...
  const loadMeasurementsForRole = useCallback(async (roleId: Id) => {
    if (!activeServer) return;
    const data = await apiJson(`/api/v1/rbac/roles/${roleId}/measurements`);
  ...
  const loadTeamMembers = useCallback(async (teamId: Id) => {
    ...
    const matches = await Promise.all(tokens.map(async tk => {
      const data = await apiJson(`/api/v1/auth/tokens/${tk.id}/teams`);
  ...
  }, [activeServer, tokens, apiJson]);

  useEffect(() => {
    if (!activeServer) return;
    resetGate();
  }, [activeServer, resetGate]);

  useEffect(() => {
    if (!activeServer || featureNotEnabled || noLicense) return;
    (async () => { setLoading(true); setError(null); try { await loadBase(); } finally { setLoading(false); } })();
  }, [activeServer, loadBase, featureNotEnabled, noLicense]);

  useEffect(() => { if (selectedOrg && !featureNotEnabled && !noLicense) loadTeamsForOrg(selectedOrg.id).catch(e => setError(e instanceof Error ? e.message : '')); }, [selectedOrg, loadTeamsForOrg, featureNotEnabled, noLicense]);
  useEffect(() => { if (selectedTeam && !featureNotEnabled && !noLicense) Promise.all([loadRolesForTeam(selectedTeam.id), loadTeamMembers(selectedTeam.id)]).catch(e => setError(e instanceof Error ? e.message : '')); }, [selectedTeam, loadRolesForTeam, loadTeamMembers, featureNotEnabled, noLicense]);
  useEffect(() => { if (selectedRole && !featureNotEnabled && !noLicense) loadMeasurementsForRole(selectedRole.id).catch(e => setError(e instanceof Error ? e.message : '')); }, [selectedRole, loadMeasurementsForRole, featureNotEnabled, noLicense]);
  ...
  const deleteOrganization = async (id: Id) => {
    try { await apiJson(`/api/v1/rbac/organizations/${id}`, { method: 'DELETE' }); setOrgs(p => p.filter(x => x.id !== id)); setTeams(p => p.filter(x => x.organization_id !== id)); if (selectedType === 'organization' && selectedId === id) { setSelectedType(null); setSelectedId(null); } } catch (e) { setError(e instanceof Error ? e.message : 'Delete failed'); }
  };
  const deleteTeam = async (id: Id) => {
    try { await apiJson(`/api/v1/rbac/teams/${id}`, { method: 'DELETE' }); setTeams(p => p.filter(x => x.id !== id)); setRoles(p => p.filter(x => x.team_id !== id)); if (selectedType === 'team' && selectedId === id) { setSelectedType(null); setSelectedId(null); } } catch (e) { setError(e instanceof Error ? e.message : 'Delete failed'); }
  };
  const deleteRole = async (id: Id) => {
    try { await apiJson(`/api/v1/rbac/roles/${id}`, { method: 'DELETE' }); setRoles(p => p.filter(x => x.id !== id)); setMeasPerms(p => p.filter(x => x.role_id !== id)); if (selectedType === 'role' && selectedId === id) { setSelectedType(null); setSelectedId(null); } } catch (e) { setError(e instanceof Error ? e.message : 'Delete failed'); }
  };
  const saveOrg = async (name: string, edit?: Organization) => {
    try { if (edit) { await apiJson(`/api/v1/rbac/organizations/${edit.id}`, { method: 'PATCH', body: JSON.stringify({ name }) }); } else { await apiJson(`/api/v1/rbac/organizations`, { method: 'POST', body: JSON.stringify({ name }) }); } setOrgModal({ open: false }); await loadBase(); } catch (e) { setError(e instanceof Error ? e.message : 'Save failed'); }
  };
  const saveTeam = async (name: string, modal: { edit?: Team; orgId?: Id }) => {
    try { if (modal.edit) { await apiJson(`/api/v1/rbac/teams/${modal.edit.id}`, { method: 'PATCH', body: JSON.stringify({ name }) }); await loadTeamsForOrg(modal.edit.organization_id); } else if (modal.orgId) { await apiJson(`/api/v1/rbac/organizations/${modal.orgId}/teams`, { method: 'POST', body: JSON.stringify({ name }) }); await loadTeamsForOrg(modal.orgId); } setTeamModal({ open: false }); } catch (e) { setError(e instanceof Error ? e.message : 'Save failed'); }
  };
  const saveRole = async (database_pattern: string, permissionsMap: Record<string, boolean>, modal: { edit?: Role; teamId?: Id }) => {
    ...
    try { if (modal.edit) { await apiJson(`/api/v1/rbac/roles/${modal.edit.id}`, { method: 'PATCH', body: JSON.stringify({ database_pattern, permissions }) }); await loadRolesForTeam(modal.edit.team_id); } else if (modal.teamId) { await apiJson(`/api/v1/rbac/teams/${modal.teamId}/roles`, { method: 'POST', body: JSON.stringify({ database_pattern, permissions }) }); await loadRolesForTeam(modal.teamId); } setRoleModal({ open: false }); } catch (e) { setError(e instanceof Error ? e.message : 'Save failed'); }
  };
  const createMeasurementPermission = async (pattern: string, permsMap: Record<string, boolean>) => {
    ...
    try { await apiJson(`/api/v1/rbac/roles/${selectedRole.id}/measurements`, { method: 'POST', body: JSON.stringify({ measurement_pattern: pattern, permissions }) }); await loadMeasurementsForRole(selectedRole.id); } catch (e) { setError(e instanceof Error ? e.message : 'Create failed'); }
  };
  const deleteMeasurementPermission = async (id: Id) => {
    try { await apiJson(`/api/v1/rbac/measurement-permissions/${id}`, { method: 'DELETE' }); setMeasPerms(p => p.filter(x => x.id !== id)); } catch (e) { setError(e instanceof Error ? e.message : 'Delete failed'); }
  };
  ...
  const bindTokenToTeam = async () => {
    ...
    try { await apiJson(`/api/v1/auth/tokens/${bindSelectedTokenId}/teams`, { method: 'POST', body: JSON.stringify({ team_id: selectedTeam.id }) }); setBindModalOpen(false); await loadTeamMembers(selectedTeam.id); } catch (e) { setError(e instanceof Error ? e.message : 'Bind failed'); }
  };
  const unbindTokenFromTeam = async (tokenId: Id) => {
    if (!selectedTeam) return;
    try { await apiJson(`/api/v1/auth/tokens/${tokenId}/teams/${selectedTeam.id}`, { method: 'DELETE' }); await loadTeamMembers(selectedTeam.id); } catch (e) { setError(e instanceof Error ? e.message : 'Unbind failed'); }
  };
  ...
  useEffect(() => {
    if (!bindSelectedTokenId || !activeServer) { setBindPermPreview(null); return; }
    (async () => { try { const data = await apiJson(`/api/v1/auth/tokens/${bindSelectedTokenId}/permissions`); setBindPermPreview(data); } catch { setBindPermPreview(null); } })();
  }, [bindSelectedTokenId, activeServer, apiJson]);
  ...
  return (
    <>
      ...
      {(noLicense || featureNotEnabled) && (
        <div className="rbac-alert" style={{ marginBottom: 12 }}>
          {noLicense ? t('views.rbac.noLicense') : t('views.rbac.rbacFeatureNotEnabled')}
        </div>
      )}
