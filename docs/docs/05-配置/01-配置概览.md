IEDB 使用 TOML 配置文件，同时所有配置项均可通过环境变量覆盖。

## 环境变量

环境变量遵循规则：**`IEDB_` + 配置路径，点 `.` 替换为下划线 `_`，全部大写**。

```
server.port          → IEDB_SERVER_PORT
storage.s3_bucket    → IEDB_STORAGE_S3_BUCKET
cluster.raft_bootstrap → IEDB_CLUSTER_RAFT_BOOTSTRAP
```

> 此规则适用于所有 TOML 配置项。设置任意 `IEDB_SECTION_KEY=value` 即可覆盖对应配置。
>
> 此外，以下环境变量由代码直接读取（非 TOML 映射）：

| 环境变量 | 用途 | 默认值 |
|---|---|---|
| `IEDB_ENCRYPTION_KEY` | MQTT 密码加密密钥（32字节，base64或hex） | 无（MQTT密码功能需设置） |
| `IEDB_DEBUG_PPROF` | 启用 pprof 调试端点 | 关闭 |
| `IEDB_DEBUG_PPROF_ADDR` | pprof 绑定地址 | `127.0.0.1:6060` |
| `IEDB_DEBUG_PPROF_ALLOW_NON_LOOPBACK` | 允许非回环地址暴露 pprof | 关闭 |
| `IEDB_COMPACTION_PROFILE` | 压缩子进程 heap 性能分析（设为 `"1"` 启用） | 关闭 |
| `AWS_ACCESS_KEY_ID` | S3 访问密钥（`IEDB_STORAGE_S3_ACCESS_KEY` 未设时的回退值） | 无 |
| `AWS_SECRET_ACCESS_KEY` | S3 秘密密钥（`IEDB_STORAGE_S3_SECRET_KEY` 未设时的回退值） | 无 |
| `AZURE_STORAGE_KEY` | Azure 存储密钥回退值 | 无 |

优先级从高到低：**环境变量 > TOML 配置文件 > 内置默认值**
...
directory = "./data/wal"
sync_mode = "fdatasync"      # fdatasync, fsync, async
max_size_mb = 500
...
### 4. 环境变量覆盖
所有 `iedb.toml` 配置项都可通过环境变量覆盖，无需修改配置文件。规则为 `IEDB_` + 路径，点替换为下划线，全大写：

```bash
# 等同于 [auth] enabled = true
export IEDB_AUTH_ENABLED=true

# 等同于 [server] port = 9000
export IEDB_SERVER_PORT=9000

# 等同于 [storage] backend = "s3"
export IEDB_STORAGE_BACKEND=s3
export IEDB_STORAGE_S3_BUCKET=iedb-prod
```

> Docker / K8s 部署时，通过环境变量注入敏感信息（密钥、Token）比写在 TOML 文件中更安全。
