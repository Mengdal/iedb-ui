...
+ Debian / Ubuntu（Debian 13+ / Ubuntu 24.04+）
+ amd64 / ARM64 架构
+ 建议 4GB 以上内存
...
+ RHEL / CentOS / Fedora（RHEL 10+ / Fedora 38+）
+ x86_64 / aarch64 架构
+ 建议 4GB 以上内存
...
**无法访问 Web 管理界面**：IEDB 启动后会从当前目录下的 `front/` 文件夹加载前端页面。前端源码及构建产物参见 [iedb-ui](https://github.com/Mengdal/iedb-ui)。将前端构建产物放在与 `iedb` 二进制同级的 `front/` 目录中：
